...
#model = torch.load('/content/drive/MyDrive/unet_100.pkl')
...
scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=4, min_lr=1e-14)
...
for epoch in range(1,num_epochs+1):
    ...
    if (epoch-1)/5 in range(1, 81):
        ...
        torch.save(model, '/content/drive/MyDrive/unet_%s.pkl' %(epoch-1))
        print('save done')
'''
#Visualize the results
if (epoch-1)/20 in range(1, 21):
