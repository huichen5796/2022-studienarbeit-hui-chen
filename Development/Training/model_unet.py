...
def GetTrainVal(batch_size):
    ...
    # instantiate dataset
    train_ds = MakeDataset(root_image, root_mask, transform=transform)
    val_ds = MakeDataset(root_image_val, root_mask_val, transform=transform)
...
every batch of train image:  torch.Size([2, 1, 1024, 1024]) torch.float32
every batch of train mask:  torch.Size([2, 1, 1024, 1024]) torch.float32
every batch of val image:  torch.Size([2, 1, 1024, 1024]) torch.float32
every batch of val mask:  torch.Size([2, 1, 1024, 1024]) torch.float32
...
#model = torch.load('/content/drive/MyDrive/model70new.pkl')
...
scheduler = ReduceLROnPlateau(
    optimizer, mode='min', factor=0.1, patience=4, min_lr=1e-11)
...
for epoch in range(1, num_epochs+1):
    ...
    if (epoch-1)/10 in range(1, 21):
        ...
        torch.save(model, '/content/drive/MyDrive/unet_model_%s.pkl' %
                   (epoch-1))
        print('save done')
